...
`openbox init --install-git-hook`, exactly like CC. Commits typed in the user's own
...
`openbox init --provider codex` writes OpenBox-owned entries for the five
events into `$CODEX_HOME/hooks.json` (default `~/.codex/hooks.json`), each
`{"type":"command","command":"\"<engine>\" hook codex <Event>","timeout":5}`
(15 s for SessionEnd, 30 s for PreToolUse; Codex timeouts are in seconds), with
...
approval decision (E9-S4); the ceiling is not a delay — the engine spends it
only when a high-risk class escalated and core filed an approval. The engine path is this running `openbox` binary
...
- **REQUIRE_APPROVAL → `deny`** (ruled **OD-SL7-ASK**). CC maps it to `ask`;
  ...
  *(E9 narrows when that fires. A high-risk REQUIRE_APPROVAL now ESCALATES rather
  than being answered locally, so the deny is what an undecided approval degrades
  to after the bounded hold, not the first thing tried. Codex has no rewake
  primitive, so a decision that lands later reaches the session through the
  advisories/findings channel rather than a system reminder.)*
...
- **Timeout clamps derived from the installed hook timeout** (OD-SL7-T2-TIMEOUT),
  **not** copied from CC's 2 s/5 s constants. Probe **P1** proved Codex kills a
  ...
  must land first. The whole-hook budget is `installedGateHookTimeout` (the
  installer's `preToolUseHookTimeoutSec`) **minus a margin**; if an org raises the
  installed timeout, the clamps scale with it. The default budgets stay
