...
// TestHighRiskClassesHaveAStableOperationID the invariant the whole approval
// loop rests on: any class the gate escalates can hold an approval, and an
// approval is keyed on activity_id; so every such class must have an operation
// identity that survives a retry.
...
// TestUngatedClassesKeepInvocationScopedIdentity a known limitation, asserted
// so it stays deliberate. An approval granted for one of these calls is
// therefore never consumed; the developer retries and a fresh request is filed
// instead.
...
// TestGatedOperationsAreDistinctPerRequest the other half: a gated class must
// not over-grant either. Approving one command must never carry to a different
// one, and core says the same about MCP arguments ("same tool with different
// arguments must require fresh approval").
...
// TestCorrelation_MCPKeepsFunctionName mCP tools keep the real function name
// on span.function because for them that field IS wire data (mcp_tool);
// tool_use_id rides metadata only.
...
// TestCorrelation_SubagentIDsInMetadata the subagent ids ride every payload
// fired inside a subagent, which is what makes the tree reconstructable
// without inventing a lifecycle type for the Subagent* boundary markers
// (coverage.md §3.2).
...
// TestCorrelation_IdentifiersBounded the new fields are structural identifiers
// and must be bounded at the untrusted boundary like every other externally-
// influenced id (maxIdentLen).
