...
func Capabilities() []providerspi.Capability {
	return []providerspi.Capability{
		...
		{Key: "telemetry.hook", Supported: true, How: "SessionStart/UserPromptSubmit/Pre+PostToolUse/PostToolUseFailure/Stop/SubagentStop/SubagentStart/PermissionDenied/StopFailure/SessionEnd → normalized events. The four failure/lifecycle hooks are new in that decision and are registered by the installer, so an EXISTING install does not fire them until `openbox init` is re-run; an older Claude Code that does not know the keys never invokes them and the events are simply absent (fail-open)"},
		...
		{Key: "tool.status", Supported: true, How: "status completed|failed on every tool ActivityCompleted, derived from WHICH hook fired — PostToolUse is documented and verified to fire only after a successful tool, PostToolUseFailure only after a failure. Never parsed from tool output; not content-gated, so it ships identically with content_capture off. metadata.is_interrupt (tri-state) separates a user cancellation from a real tool failure — both are `failed`, and an operator needs to tell them apart"},
		{Key: "commit.binding", Supported: true, How: "OpenBox-Session git trailer; provider-independent"},
		{Key: "telemetry.tokens", Supported: true, How: "PER TURN (ResolveFinops, default on): Stop/SubagentStop → llm_completion activity pair carrying model + 4 token counts, plus the SessionEnd session rollup. Window sums, not per-model-call — hooks fire per turn. Transcript projection binds one egressing string (model), allowlist-enforced (INV-2); off the hot path. Cost never derived here"},
		...
		{Key: "verdict.apply", Supported: true, How: "enforce leg: every gated PreToolUse call AND every UserPromptSubmit prompt is evaluated by /evaluate → permissionDecision deny/ask on tools, decision:block on prompts; ON by default; tighten-only, never emits allow as a grant; REQUIRE_APPROVAL is held for a real decision and denies/blocks if unanswered. A HALT the control plane returns additionally stops the session: continue:false now, and a local latch refuses every later prompt/tool call in that session (resume included). Prompt gating and the raised UserPromptSubmit timeout are installer-registered — an existing install needs `openbox init` re-run"},
