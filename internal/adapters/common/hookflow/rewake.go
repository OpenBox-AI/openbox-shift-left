...
const (
	...
	//
	// That cost is paid far more often since. The watcher used to start only for
	// shell and MCP calls; every gated class evaluates inline now, so a session
	// doing many small tool calls holds one idle watcher per call for up to this
	// grace. The trade is unchanged in shape — an idle process nothing waits on,
	// versus an approval that can never wake the session — but it is no longer a
	// rare path, and that is the number to revisit first if watcher count ever
	// becomes the problem.
