...
Every file here is an **illustrative copy**. Nothing in this directory is
loaded, installed, embedded, or consulted at runtime.

## The authority is `internal/cli/laneservice`

A unit that actually lands on a machine is **rendered at install time** with
that machine's binary path, its `HOME`, the address the developer chose, and a
stop timeout matched to the daemon's own shutdown-grace value.
`kardianos/service` owns the render; `internal/cli/laneservice` owns the `Spec`
that drives it.
...
An extracted `go:embed` copy would be a **second store of derivable state** with
a sync obligation, and the drift would be invisible in exactly those two ways.
This repository has already made that mistake once and reverted it: the lane
election was written as a stored field in `dev.json`, tested green, and backed
out, because a second copy of derivable state drifts silently in the worst
direction.
