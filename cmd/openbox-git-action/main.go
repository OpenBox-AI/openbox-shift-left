...
// obx_(live|test)_… runtime key (INV-1: never logged) OPENBOX_DID the agent's
// did:aip:<uuid> OPENBOX_AGENT_PRIVATE_KEY base64 raw 32-byte Ed25519 signing
// key Exit codes: 0 = resolved (emit success OR fail-open drop, INV-3); 2 =
// usage / precondition fault the operator must fix (bad --sha, missing creds
// when not --dry-run).
package main
