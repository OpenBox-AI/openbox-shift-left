...
onboard with `openbox dev init`, pull rules with `openbox dev sync`, start the
sidecar, then run `claude` — all three sidecar tiers fire from **synced backend
rules**, and a commit flows through to **commit → session → deploy lineage** in
the UAT dashboard.
...
**You run locally:** the sidecar + Claude Code only (core/backend are deployed UAT).
...
## The three sidecar tiers (each a distinct on-camera beat)

Wired in `adapters/claude-code/hookrun.go` (`RunHook`).

| Tier | Trigger | Rule source | Observable |
|---|---|---|---|
| **1 · local secret redaction** | a `Write`/`Edit` with a secret in the body | local detector (`sidecar/secrets.go`), always on under enforce | written file shows `${OPENBOX_REDACTED_…}`; secret never egresses |
...
`dev sync` localizes the policy, but the local sidecar input has **no
...
- **Every Claude Code session on the box gets "request denied … fail-closed".**
  Enforcement was written to the **global** `~/.config/openbox/dev.json` with no sidecar
  running. Never do that — enforce belongs in the **demo terminal's env** (RUN.md step 0),
  scoped to that one `claude` session. To recover: clear `enforce`/`fail_closed` from `dev.json`.
...
- **Nothing enforced.** Sidecar not running, or the step-0 env wasn't exported in the
  terminal you launched `claude` from.
...
- **Sidecar won't bind.** Socket dir must be `0700` and the path short (<~108 chars).
