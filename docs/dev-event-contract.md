...
OD4's original metadata-only-by-default posture). Prompt content is captured and
...
- ~~Tool commands and file bodies never egress on **observe** events
  (SL3-SEC-3).~~ **Retired in v1.3**. Tool input, tool output and the free-text
...
- **SL-3 (client):** build the base-SDK wire payload per MAPPING.md; `import`
  the `conformance` package to validate outbound events before signing/POST.
- **Adapters (SL-4/7/8):** map native tool payloads onto this schema in
  `emit()`.
