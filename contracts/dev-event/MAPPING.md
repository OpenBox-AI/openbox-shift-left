...
| `PermissionDenied` | `SignalReceived` | `permission_denied` | — | `tool_name`, `tool_use_id`, `permission_mode`? | mid-session signal: THAT a call was refused and which tool it was about. Never why — the provider's `reason` is free text and is unbound (ADR-0019 owns it) |
| `APIError` | `SignalReceived` | `api_error` | — | `error_type` (closed provider enum) | mid-session signal; a turn that ended in a provider error rather than an answer |
