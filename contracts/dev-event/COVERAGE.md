...
- **`ToolCall`↔`ToolResult` correlation**: carry the provider's `tool_use_id` in `metadata` (all three expose it) and reuse one `span_id` across the pair. Both shipped adapters do this by id rather than by heuristic — the id rides `span.invocation_id`, a local field that never egresses. A new adapter must also supply `span.operation_id` for any class it lets the gate escalate, or an approval cannot survive a retry; see MAPPING.md "Operation vs invocation identity".
