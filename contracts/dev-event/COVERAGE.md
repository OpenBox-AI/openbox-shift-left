...
2. **Subagent lifecycle** (`SubagentStart`/`SubagentStop`) — the start/stop markers are telemetry-only (metadata), not a distinct lifecycle type. A subagent is a nested actor, **not** a `tool.kind` — no new kind is needed. Correlation fields (`metadata.agent_id`/`agent_type` on a subagent's tool calls) land with **E8-S3**; before that story the Claude Code adapter parses none of them.
