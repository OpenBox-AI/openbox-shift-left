...
// syncShim keeps the call shape these tests were written against — they
// predate the extraction, when sync was a method on the CLI's app struct with
// the reader behind a seam. Preserving the shape keeps the diff to the move
// itself rather than mixing in a test rewrite.
type syncShim struct{ reader PolicyReader }

func syncApp(r PolicyReader) (syncShim, *bytes.Buffer, *bytes.Buffer) {
	return syncShim{reader: r}, &bytes.Buffer{}, &bytes.Buffer{}
}

func (s syncShim) syncPolicyBundle(ctx context.Context, _, _, _, agentID, bundlePath string, out io.Writer) error {
	return Run(ctx, s.reader, agentID, bundlePath, out)
}
