...
//
// PATHS ARE TODAY'S, AND PHASE 03 MUST HAND-EDIT THEM.
//
// The rewrite instrument matches only the contiguous qualified form
// `github.com/openbox-ai/openbox-shift-left/<path>`. Nothing in this file is in
// that form: subtree names are bare (`"decision"`), allowlist entries are built by
// concatenation (`repoPrefix + "/client"`), and roots are assembled with
// filepath.Join. So the collapse will NOT update this file, and the failure is
// loud but misleading -- both the unallowed and dead-entry tests fire at once,
// which reads as "the guards broke" rather than "the move missed a file".
//
// The hand-edits are: the four `name:` fields, the two adapter pair paths, the
// five `repoPrefix + "/x"` entries, and the conformance filepath.Join. Then RE-RUN
// THE DRILLS on the collapsed tree -- the phase requires them red after the move,
// not only before, and the one that matters is gateway importing
// internal/cli/devinit.
...
func TestConformanceClosureIsReviewed(t *testing.T) {
	dir := filepath.Join(repoRoot(t), "contracts", "dev-event", "conformance")
...
func TestAdaptersDoNotImportEachOther(t *testing.T) {
	...
	for _, pair := range []struct{ subject, forbidden string }{
		...
		{"adapters/codex", "adapters/claude-code"},
	} {
