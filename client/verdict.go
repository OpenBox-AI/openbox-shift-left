...
//
// It prefers approval_id and falls back to governance_event_id. That order is
// aspirational in the first field and load-bearing in the second: core declares
// approval_id but assigns it only in tests, so a reference built from it alone
// was always empty and the code that quoted it was dead (E9 §1.3 defect 1).
// governance_event_id is a REQUIRED field of core's public verdict response and
// is what the approvals queue is keyed on, so it is the reference that actually
// resolves. Both are server correlation ids, not tool content (INV-2).
...
// verdictOnly is the smallest shape that can carry a decision. Decoding into it
// ignores every other field in the body whatever its type, which is what makes
// the fallback in parseEvaluation worth having.
...
//
// It decodes into verdictOnly rather than the rich verdictResponse. Using the
