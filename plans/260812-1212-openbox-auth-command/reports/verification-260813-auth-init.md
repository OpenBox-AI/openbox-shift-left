...
| **High** | **Installers resolved the dev.json *write* target through the read path.** `DefaultConfigPath()` prefers an existing *legacy* file over a not-yet-created new one, so with migration failed (explicitly non-fatal) an install would write into the pre-ADR-0015 directory. Worked only by coincidence of ordering. | Both installers use `DevConfigWritePath()`. |
...
- **`org` is not persisted.** The plan's field table said `org` → `dev.json`, but
  ...
  (which `CLAUDE.md` says needs an ADR). It is used to derive an agent name at
  registration time and passed per run via `--org` / `OPENBOX_ORG`, exactly as
  `init` did. Cost: a re-run of `auth` does not remember the org. Worth an ADR if
  that friction matters.
- **ADR-0016's `*bool` rationale was half wrong and is now corrected.** The plan
  (and the ADR's first draft) claimed `resolveBool` never reaches its default
  argument. A probe showed it does — the key-presence map in
  `resolveBoolWithSource` already handles the plain-bool accessor. The real and
  sufficient reason for the type change is the **write** side: `omitempty` drops an
  explicit `false`, which would have made `--enforce=false` silently
  un-appliable. The ADR now records the correction and the general rule (check both
