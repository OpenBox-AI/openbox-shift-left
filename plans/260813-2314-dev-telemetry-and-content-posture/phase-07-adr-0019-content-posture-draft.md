# Phase 07 — ADR-0019: full-content-capture posture draft (Proposed)
...
- Style precedent: ADR-0015 (argues against the prior rationale, not around it), ADR-0014
  (allowlist amendment mechanics), ADR-0017 (posture-change disclosure)

## Overview

- **Date:** 2026-08-13 · **Priority:** P1 · **Status:** pending · **Effort:** 3h
- Draft `docs/adr/ADR-0019-full-content-capture.md` (**Proposed** — stays Proposed until owner
  accepts) authorizing the P1–P3 capture phases. **DRAFT ONLY — no P1–P3 implementation in this
  plan.** Cross-references ADR-0018 as the posture's first shipped increment.

## Key Insights

1. The ADR's hard job is retiring SL3-SEC-3 and restating INV-2 honestly: from "content has no
   field to land in" to "content egresses only under the org gate, always secret-redacted first,
   always 64KB-capped". Must argue against the original metadata-only rationale (ADR-0015 style),
   citing the owner's org-deployment trust model (company machine, company code).
2. Thinking capture requires amending ADR-0014's transcript allowlist — fold the amendment INTO
   ADR-0019 (one decision record for one posture change); the sentinel-test rule stays: the test
   evolves to assert redaction+cap instead of absence, and a trivially-passing version remains a
   defect.
3. ADR-0018 already shipped part of the posture (status; assistant final text via the span
   stopgap). ADR-0019 must position itself as the umbrella: P2 moves assistant text to
...
4. `docs/data-and-privacy.md` rows flip from "never" to gated — the rewrite list is part of the
   ADR's Consequences, file by file.
...
1. **Context:** org-governance deployment model; owner decision 2026-08-13; what evaluation needs
   ...
   report; ADR-0018 as shipped increment.
...
6. **Consequences:** doc rewrites (data-and-privacy.md, MAPPING.md, COVERAGE.md, README,
   ...
   activity_output ask (links phase 04's issue — also retires ADR-0018's synthesized keys);
...
7. **Sequencing:** P1 tool output → P2 assistant text on `activity_output.message` + `stop_reason`
   + the freed P0-deferred strings → P3 thinking + observe-path inputs + the ADR-0014 allowlist
   amendment (staged last because the sentinel is load-bearing). Each phase lands with its
   conformance case before flush code.

## Related code files

- `docs/adr/ADR-0019-full-content-capture.md` (new), `docs/adr/README.md` (index row),
  `docs/adr/ADR-0014-…md` (amendment cross-note in header pointing at 0019)
- Referenced, unmodified: `client/event.go` (Content), `client/payload.go`
  ...
  `contracts/dev-event/MAPPING.md`, `docs/adr/ADR-0018-…md`

## Implementation Steps

1. Draft per outline; every claim cites a repo symbol/path or the research report.
2. Cross-check no contradiction with ADR-0013/0014/0015/0016/0017/0018 "worth not re-litigating"
   lists; where ADR-0014 is amended, say so in both files (note in 0014's header pointing to
   0019). Verify the ADR-0018 relationship reads as increment-then-umbrella, not conflict.
3. Index row in `docs/adr/README.md`; keep ≤ house length (~0017 size).
...
5. Commit: `docs(adr): propose ADR-0019 full-content-capture posture`.

## Todo list

- [ ] ADR-0019 drafted per outline (Proposed)
- [ ] ADR-0014 amendment cross-note; ADR-0018 increment cross-reference
- [ ] README index row
- [ ] Owner review requested

## Success Criteria

- ADR self-consistent with the research report, the decision memory, and shipped ADR-0018; names
...
| Risk | L×I | Mitigation |
|---|---|---|
| Scope creep into implementation | M×M | This phase produces exactly three file changes under `docs/adr/` |
...
- The ADR makes redaction the load-bearing control: state the ordering requirement (detect →
...
Plan complete. After owner accepts ADR-0019 → new plan for P1 (tool output). Open follow-ups for
other repos: the filed AGE activity_output ask (phase 04); core success derivation dead for the
Python SDK producer too; server-side dedupe for the lost-200 window; retention/PII posture for
the new content classes (backend).
