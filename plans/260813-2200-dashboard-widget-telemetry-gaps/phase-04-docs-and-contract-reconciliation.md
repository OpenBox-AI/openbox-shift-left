...
- Authority for the changes:  (phase 01)
...
1. **Two paragraphs go from true to false**, and both are load-bearing privacy claims:
   - `docs/data-and-privacy.md:24-34`: "The span is now gone entirely, so the fields are
...
either still true or rewritten, with that decision link.
...
docs/data-and-privacy.md short-version table, that decision span paragraph,
...
| `docs/data-and-privacy.md:24-38` | rewrite that decision paragraph: what changed, that a response-body channel exists again for one carrier, that it is redacted before send, and that `secret_detection:false` means unredacted |
...
| `CLAUDE.md:23-25` | "Dev sessions write no `spans` rows" → no spans EXCEPT the one turn-content carrier |
...
| `CLAUDE.md` Current state | one paragraph for that decision with the four non-re-litigable points (see steps) and an honest status line |
...
(gate, redaction, cap). Cite.
5. `docs/architecture.md` assurance bullets.
6. `README.md`: grep for `span`, `telemetry`, `metadata`, `assistant`, `never`; edit only what is
   actually asserted.
7. CLAUDE.md last (it summarises the others). The that decision paragraph should carry the points worth
   not re-litigating:
...
9. Commit: `docs: reconcile the span-less and tool-output claims with that decision` +
...
- [ ] `docs/data-and-privacy.md` table row + that decision paragraph rewrite + capture section
...
| CLAUDE.md grows past usefulness | M×L | One paragraph, four bullets, one status line — the shape that decision/0014/0017 paragraphs already use |
