...
| OS-00-03 | verified | Native drivers, profiles, probes, receiver, scenarios, governed rerun, trusted native fixture path, and producer-only analysis APIs are removed. Passive evidence/reporting and the poison/sink/Ollama relay remain. **Superseded at release:** Phases 1–4 reimplemented the sink (`evaluate/effect_relay.go`), moved inference onto OpenShell's own provider route, and moved candidate validation into `securityreport.Prepare`, leaving the retained `assurance/fixture` and `assurance/securityanalysis` packages orphaned with zero importers. Both were dropped before the first commit and are absent from history; the poison fixture belonged to the retired scenario machinery and `retrieval_poison` is now a declared `missing` coverage channel. |
