# Phase 01 — Contract: turn-as-activity, the ADR, and the core issue
...
- Precedent: `docs/adr/ADR-0013-*` (span layer retired; tool call = activity),
  `docs/adr/ADR-0004-*` (base wire unification)
...
- Description: extend the dev-event contract with the turn pair, widen `Tokens`,
  write the ADR that records the INV-2 allowlist, and file the one core-side
...
- The ADR is load-bearing, not ceremony: `usage.go`'s stated guarantee is that
...
7. Update `contracts/dev-event/schema/dev-event.schema.json` (+
   ...
   live-run checklist).
8. Write the ADR (turn-as-activity + INV-2 identifier allowlist) and **file the
...
| `docs/adr/ADR-00NN-turn-as-activity-and-identifier-allowlist.md` | new |
...
7. Write the ADR. State: what INV-2 guaranteed before, what it guarantees now
   (exactly one egressing string, identifier-class, bounded, test-enforced), that
   ADR-0013 stands unamended and this rides the activity shape it established,
...
8. File the openbox-core issue: activity-based extractor for
   ...
   `ModelMetric`/`UpdateModelUsageActivity` (provider derived from model id, since
   an activity has no `http_url`); add `<model>.cache_creation_tokens` /
   `<model>.cache_read_tokens` composite keys; exclude `llm_completion` from
   `ExtractToolMetric`; `unknown` bucket for model-less rows; note the
   pricing-table gaps for current Claude Code model ids. Link the issue from the
   ADR and `docs/architecture.md`'s assurance section.
...
- [x] ADR written, cross-linked from `docs/architecture.md`
...
- ADR exists and says out loud that INV-2's usage path is now an allowlist.
...
| `activity_output` grows a second string "while we're here" | medium | **high** | ADR names `model` as the sole entry; Phase 03's sentinel test fails on any other bound string |
...
- The ADR is the deliverable that keeps the repo's privacy claims true; write it
