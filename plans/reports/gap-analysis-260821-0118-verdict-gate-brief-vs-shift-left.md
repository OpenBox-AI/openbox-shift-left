...
**Method:** clause-by-clause read of the brief against code (file:line cited), ADRs, and the prior verification
report `plans/reports/verification-260820-1854-engineering-brief-claims.md` (executed-proof facts reused).
...
would violate this repo's core principle (reuse the existing `/evaluate` pipeline; new endpoint/service ⇒ ADR).
...
| monitor / enforce | observe (`enforce:false`) / enforce (default ON since ADR-0016) |
...
2. **Decision output contract** (brief §4.2, graded [C]). Implemented and conformance-pinned on real stdout bytes:
   ...
   Beyond the brief: HALT renders `continue:false` + stopReason (session kill, ADR-0020) and a local halt latch
   refuses every later gated hook in that session.
3. **ALLOW = defer** (brief ADR-2). Proceed path writes *nothing* — byte-identical to observe
...
4. **Gate everything, decide server-side** (brief ADR-3). ADR-0017: no local verdicts, no client pre-filtering,
...
7. **Fail-closed enforcement** (brief §4.3 core differentiator). Capability shipped: `ApplyFailurePolicy` runs
   ...
   conformance-tested. Deltas: **default is fail-open** (`failurepolicy.go:11-17`, ADR-0016) — see conflict D1;
   no persisted circuit-breaker/degraded state (per-call policy only) — see gap G6.
8. **Approvals** (brief §7). REQUIRE_APPROVAL → request filed by control plane → bounded hold
   ...
   (approve→proceed, reject→deny) or undecided→**deny with approval ref** → **rewake watcher**
   (async, 2700s, `rewake.go:23`) wakes the session when the decision lands. This is the brief's Mode B +
   §7.3 fallback, already engineered around the hook-timeout ceiling the brief flags as an M0 risk — without
   depending on a 300s hook timeout. Plus the **autonomous approver** (ADR-0012, `openbox approve`/`approveauto`)
   the brief doesn't have. Delta: approval surface is dashboard/CLI, not Slack — gap G2.
9. **Prompt handling** (brief §4.4 wants hash-and-record). Repo goes further: UserPromptSubmit **gates**
   (ADR-0020, same EnforceGate; block/erase, hold, HALT→session stop) and prompt text is captured under
   `content_capture` (default ON). Delta on hashing posture — conflict D4.
10. **Outcome telemetry** (brief §4.4 PostToolUse). Shipped richer: tool ActivityCompleted with
    `status: completed|failed` (ADR-0018), PostToolUseFailure/PermissionDenied/StopFailure signals, token usage +
    model per turn (ADR-0014), one content span per turn for alignment (ADR-0018).
...
14. **Enterprise distribution** (brief §4.1). Managed-config posture layer with `locked` fields +
    `deploy/managed/` profiles; global scope prints the managed-settings snippet (ADR-0016 scope semantics).
...
18. **HTTP-hook vs command shim** (brief ADR-1). Same conclusion already embodied: command hooks own
...
|---|---|---|---|
| D1 | `enforce` defaults **fail-closed**; "the differentiator" | Default **fail-open** (ADR-0016 mitigation: `fail_closed` stays off; OD-E8-1 unratified). Worse: shipped managed profile **locks** `fail_closed:false` making it unreachable for that fleet (SL-2, proven) | Direct conflict. Owner call. If the brief's positioning stands, minimum change = unlock in `deploy/managed/openbox/dev.json`; default flip = ratify OD-E8-1 |
...
| D3 | New **Gate API** service + `/v1/gate/*` schema | Core principle: reuse `/evaluate`, same tables, same auth; new endpoint/service ⇒ ADR. All five proposed endpoints have existing equivalents (§0 table) | Conflict with repo's reason to exist. The brief's [A] "reconcile with existing contracts" resolves to: they exist, use them |
| D4 | `store_inputs: hashed` default; raw input never leaves host | Content capture **ON by default** (owner, 2026-07-15; memory pin: don't re-litigate metadata-only). Gated calls egress redacted content ≤64KB (ADR-0017) — "OpenBox cannot decide on content it cannot see" | Conflict. Brief §6 re-litigates a settled posture. Note: brief's own policy pack (curl-pipe-sh detection, §8) *requires* content visibility its default denies the server |
| D5 | Token via env "or OS keychain ref" | Keychain **deleted, not demoted** (ADR-0015): plaintext `~/.openbox/.env` 0600, argued against the prior rationale | Conflict if read as a requirement; agrees with repo if env-only |
...
- **G4 — Default policy pack incl. self-protection.** No shipped pack at all ("policy template packs" =
  ...
  disclaimed (ADR-0015: origin-of-config, not tamper-resistance). **G4b:** SL-1 (proven) — `base_url`/
...
Provider-agnostic engine + **Codex adapter** (brief is CC-only); session **HALT kill + latch** (ADR-0020);
prompt **gating** (not just hashing); autonomous approver (ADR-0012); token usage/model finops telemetry
(ADR-0014); tool outcome + failure signals + alignment span (ADR-0018); goal-alignment feed; AIP request
signing; installer stale-engine sweep + `openbox doctor` duplicate-engine diagnostics; Windows cross-compile;
`attest`/lineage read side; managed posture layer with provenance (`decision_authority` on the wire).
...
1. D1 — does the fail-closed-by-default positioning win over ADR-0016's fail-open default (and OD-E8-1)?
