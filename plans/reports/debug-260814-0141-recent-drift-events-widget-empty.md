...
1. **Drift verdicts are produced only on span-less events.** Alignment session logic (`openbox-core/internal/services/age.go:109-172`): trace check runs only on `SignalReceived` with non-empty `signal_args` (= PromptSubmitted, 2nd+ prompt) and `WorkflowCompleted/Failed` (= SessionEnded). All other events (incl. That decision turn-span carrier) only append assistant content to the Redis session — `checked=false`. Trend's "5 evaluations / 3 drifted" = prompt + session-end checks.
...
5. Dev sessions can't even reach the hook path by accident: shift-left never sets `hook_trigger` (deliberate, `client/payload.go:186-195`), and tool events are span-less. Even on the hook path (base SDK), the event-level row is written with whatever the AGE result says — and alignment returns `checked=false, drifted=false` on span-carrying events — so `goal_drift=true` rows are never produced there either.
