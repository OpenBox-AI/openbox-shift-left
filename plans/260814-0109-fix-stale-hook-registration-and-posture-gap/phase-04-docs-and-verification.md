...
- **Date:** 2026-08-14
- **Description:** Update every doc claim the three fixes change, confirm that decision needs no
...
- No new decision record is required: no new table, endpoint or service (CLAUDE.md "Core principle"). Issue 1
is a bugfix inside an existing merge; Issue 2 makes shipped code match that decision:237-239,
which is why D2 chose the key over a decision record amendment. **If D2 is reversed, this phase
changes shape** — that decision:237-239 would need an amendment saying `fail_closed` carries the
failure policy.
- decision records are historical records, not living docs. That decision:222-252 and that decision:51,229 are **not**
rewritten; that decision's `localhooks.go:45-55` line citation will drift after Phase 01 and that
is acceptable for a dated decision record — verify, do not chase.
...
untrue since that decision deleted the bundle. Pre-existing, but it is a bullet in the exact
list Phase 01's behavior change edits, and this repo's rule is that docs stay true. Fix it in
the same edit; it is one line.
...
3. `openbox doctor`'s documented capability list mentions the duplicate-engine warning (only if
   Phase 03 shipped).
4. That decision verified accurate as-written; no decision record text edited unless D2 is reversed.
...
| | 150 | delete the dead "pulls your org policy into a local bundle" bullet |
...
| — | 222-252 | verify accurate after Phase 02; **no edit** under D2 |
| — | 51, 229 | verify only; stale line citation accepted (dated record) |
| `CLAUDE.md` | "Current state" | 2-4 sentences: init now replaces its own stale-path registrations (and why the live 66-event double-count was two engines, not an that decision defect); posture now carries `decision_authority`/`failure_policy` and the five bundle keys are gone |
...
5. Verify `docs/data-and-privacy.md:99,140` and both decision records; record "verified, no edit" per file
...
- [x] `docs/data-and-privacy.md`, that decision verified (no edit expected)
...
- that decision:237-239 is a true statement about shipped code, unedited (D2 held).
...
| R2 Editing a decision record turns a dated record into a living doc | Med×Med | decision records are verify-only here. The one exception is a D2 reversal, which would require an explicit amendment section, not an in-place rewrite. |
...
- No doc edit may imply `settings.local.json` or `~/.openbox/.env` is protected; the
