...
- **Priority:** P2 · **Implementation status:** not started · **Review:** not reviewed
...
- [ ] `test/` moved, 95 references updated, preflight fails only on the stack
- [ ] `deployments/` moved, MDM recipe walked
- [ ] schema in `api/`, `schemaRelPath` fixed, **38 conformance cases execute**
- [ ] `api/` holds no `.md`
- [ ] `init/` populated, docs-only; `grep -r 'go:embed' init/` empty
- [ ] contract prose in `docs/`; `ext-core/` deleted; links fixed
- [ ] full green gate
