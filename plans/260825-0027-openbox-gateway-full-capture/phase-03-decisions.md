# Phase 03 — Decisions, decision records, and probes
...
- Repo rule: `CLAUDE.md` — "A new table, endpoint or service requires a decision record in "
- Gate for: phases 04–08. **Nothing in Track B starts until this closes.**

## Overview

- Date: 2026-08-25 (local-gateway revision)
- Description: accept that decision, write the gateway decision record for the LOCAL topology, amend
That decision's install scope, run the three probes that shape Track B, and open
the account-registry conversation with the backend.
- Priority: P1
- Implementation status: **partially prepared** — every artifact an unattended
session can produce is written; the three probe RUNS, that decision's
acceptance, and the backend filing are user actions (see Todo)
...
- **The install-scope conflict is still real.** That decision defaults `init` to project scope;
`ANTHROPIC_BASE_URL` is read only from managed settings and `~/.claude/settings.json` where
the Desktop app manages the connection, and background agents need settings rather than
shell exports. Amending that decision remains a prerequisite for phase 07 — the value now
points at localhost instead of an org host.
- **The assurance story must be written as tiers, in that decision, before code.** Base =
...
the same binary, not built now. a decision record that blurs these into one claim repeats
the overstatement failure this product exists to prevent.
...
surface on their side — their decision record rule, their call; this phase files the
ask with the evidence contract (what sensors will send).

## Requirements

1. That decision moved Proposed → Accepted (P1/P3 land in phases 01–02).
2. New that decision: OpenBox gateway as a **local per-developer service** — substitution
...
3. That decision amendment: gateway env config installs at user or managed scope.
...
`/evaluate` is unreachable, regardless of `fail_closed`; no offline grace. That
decision records the divergence from hook-path posture and its cost.
8. Backend ask filed: account registry as policy input + evidence contract.

## Architecture

No production code. Three probes against the real installed binary, three decision record
documents, one backend ask. Probes A and P0 share the same throwaway HTTP server; P1 is
token/header inspection plus a local-state read.
...
|---|---|
| — | status → Accepted |
| — | new — local topology, tiers, pass-through (**drafted**; `local` is in the filename because it is the decision) |
| — | amendment: scope for gateway env config |
...
4. Accept.
5. Write that decision per requirement 2, including what the gateway explicitly cannot do:
...
6. Amend.
7. Record the validated always-refuse posture in that decision — the hook/gateway divergence,
...
- [x] `that decision` amendment **drafted** (user/managed scope for the gateway
...
- [ ] **USER: accept that decision** (owner signature — that decision's own Acceptance section
...
- [ ] Fill that decision–10 from the probe reports, then review it (tier split is the
      decision — do not merge if the tiers read as one claim)
- [ ] CLAUDE.md current-state updated once that decision is accepted
...
- that decision states the three assurance tiers in plain language and never claims prevention
  for the base tier.
...
|---|---|---|---|
| P0 negative for OAuth | pass-through still covers API-key orgs | subscription session ignores BASE_URL | scope statement in that decision + product docs: gateway tier requires API-key/console auth; Track B proceeds |
...
| that decision blurs the tiers | reviewer checklist names all three | decision record reads as one assurance claim | do not merge; the tier split IS the decision |
...
- that decision must state plainly that with pass-through auth **OpenBox holds no provider
credential anywhere** — and that this is why the custody claim is gone. The fingerprint is
one-way; that decision records the hash construction and that raw credentials never
egress.
