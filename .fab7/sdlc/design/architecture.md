...
| Telemetry | OTel `WorkflowSpanProcessor` → OTLP exporter (`span_processor.py`) | Claude Code **native OTel** → **same OTLP collector**, mapped to `SpanEntity` |
...
1. **Developer onboarding interface (edge, per-provider).** The developer installs a thin OpenBox **adapter** for their tool and authenticates it with an `obx_` key + developer DID — exactly the `create_openbox_worker()` gesture, one layer out. Governance is then *ambient*: the developer keeps using their tool normally; there is no new day-to-day UI. **Front door (DECIDED, OD12): a unifying `openbox` CLI** — `openbox dev init --provider claude-code|cursor|codex` detects the tool, writes its native config (Claude Code plugin/settings; Cursor `hooks.json`/Team-hook bundle; Codex config), and registers the session-as-agent. The native packages remain the substrate, so admins can still force-deploy via managed settings / Team hooks.
...
- **Advisory** — + guardrail/policy signals recorded but not blocking.
...
- FR-2 ingest Claude Code OTel (tokens/cost/tool decisions/MCP) → **reuse** the SDK's OTLP path (`span_processor.py`) → `SpanEntity`.
...
**Nonfunctional (PRD §3):** NFR-1 privacy metadata-default (content off, gated OD4/S4); NFR-2 <50 ms via Claude Code async hooks + OTLP push; NFR-3 observe fail-open; NFR-4 attribution (S3); NFR-5 org mandate via managed settings, pilot opt-in (OD10); NFR-6 reuse `AgentEntity.createHash`; NFR-7 retention (OD4).
...
- **D2 — Reuse the existing OTLP/span path.** Claude Code native OTel (`CLAUDE_CODE_ENABLE_TELEMETRY`, `OTEL_EXPORTER_OTLP_ENDPOINT`, S1 A4) targets the same collector the SDK's `WorkflowSpanProcessor` forwards to; spans land as `SpanEntity` rows linked to the governance event. *(Resolves former OD-B → reuse.)*
...
- **D4 — Extend the event-type enum, don't fork the schema.** Add developer-runtime types alongside the existing Temporal ones (`content.EventTypeWorkflowStarted…Handoff`, `governance.go:273` `isValidGovernanceEventType`): `SessionStarted`, `PromptSubmitted`, `ToolCall`, `ToolResult`, `SessionEnded`, `CommitCreated`, `Deploy`. This is the one required server change and it is additive.
- **D5 — Identity: new agent `kind`, same everything else.** `POST agent/create` with `kind=developer`; issue `obx_` key + `didFor()` + Ed25519 signer material, mirroring SDK onboarding. Session ephemerality is captured by `SessionEntity` under the developer agent's DID.
...
- **Async/best-effort on the hot path (NFR-2/3):** adapters emit asynchronously (Claude Code `"async": true`); OTLP push is non-blocking; no synchronous dependency on OpenBox for a tool call to proceed in Phase 1.
...
| **OD12** | **DECIDED (2026-07-07): unifying `openbox` CLI** as the front door — `openbox dev init --provider <tool>` detects the tool, writes its native config, and registers the session-as-agent. Underlying packages stay native so admins can still force-deploy via managed settings / Team hooks. | brian (product/UX) | Resolved. |
