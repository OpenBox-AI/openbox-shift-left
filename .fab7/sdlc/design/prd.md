...
| **FR-2** | The system ingests Claude Code session telemetry via native OpenTelemetry (metrics + events): token usage, cost, tool decisions, MCP connections, commit count — keyed by `session_id` and `prompt_id`. | S1 A4 (code.claude.com/docs/en/monitoring-usage.md); brief option A1 | An OpenBox OTLP collector endpoint + a normalized session-event store. |
