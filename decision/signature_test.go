...
// Hooks are separate short-lived processes, so two can pin at once. The
// read-then-write was not atomic across them: a process pinning a lower epoch
// could land last and walk the rollback floor back down.
func TestWriteEpochPin_ConcurrentWritesNeverLowerTheFloor(t *testing.T) {
	dir := t.TempDir()
	bundlePath := filepath.Join(dir, "bundle.json")

	var wg sync.WaitGroup
	// Interleave high and low epochs from many goroutines.
	for i := 0; i < 50; i++ {
		for _, epoch := range []int64{7, 5, 3, 9, 1} {
			wg.Add(1)
			go func(e int64) {
				defer wg.Done()
				WriteEpochPin(bundlePath, e)
			}(epoch)
		}
	}
	wg.Wait()

	if got := ReadEpochPin(bundlePath); got != 9 {
		t.Errorf("epoch pin = %d, want 9 — the floor may only advance", got)
	}
}

// A pin write must never leave a torn or unreadable file behind for the next
// process to parse as 0 (which silently re-enables rollback).
func TestWriteEpochPin_LeavesNoPartialFile(t *testing.T) {
	dir := t.TempDir()
	bundlePath := filepath.Join(dir, "bundle.json")
	WriteEpochPin(bundlePath, 42)

	if got := ReadEpochPin(bundlePath); got != 42 {
		t.Fatalf("epoch pin = %d, want 42", got)
	}
