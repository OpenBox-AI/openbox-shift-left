...
// OD-RF-3. An org that has deployed signing can require it, turning the
// signature from detection into prevention.
//
// Without this, the threat model's own scenario wins cheaply: an attacker with
// local file write does not need to forge a signature, they delete the `signed`
// block and edit the rules, and the tampered policy is enforced with the
// integrity state merely recorded. With it on, that bundle is not loaded at all.
func TestRequireVerified_RefusesAnUnsignedBundle(t *testing.T) {
	...
	// Default posture: an unsigned bundle is enforced, and its rule fires.
	lenient := NewInProcessDecider(InProcessConfig{BundlePath: path})
	if got := lenient.Decide(context.Background(), toolCall("Bash", client.ToolShell, nil)); !got.Evaluation.WouldBlock() {
		t.Fatalf("baseline: an unsigned bundle should still be enforced, got %+v", got)
	}

	// Requiring verification: the same bundle is not loaded, so the decider is
	// at cold-start and the failure policy decides instead.
	strict := NewInProcessDecider(InProcessConfig{BundlePath: path, RequireVerified: true})
	got := strict.Decide(context.Background(), toolCall("Bash", client.ToolShell, nil))
	if got.Evaluation.WouldBlock() {
		t.Error("an unverified bundle must not be enforced when the org requires verification")
	}
	if !got.FailOpen {
		t.Errorf("refusing the bundle must leave the decision fail-open so the failure policy governs, got %+v", got)
	}
	if strict.Integrity() == IntegrityVerified {
		t.Error("integrity should report why the bundle was refused")
	}
}

// A genuinely verified bundle still loads under the strict posture — otherwise
// the setting would just disable enforcement.
func TestRequireVerified_LoadsAVerifiedBundle(t *testing.T) {
	pub, priv, err := ed25519.GenerateKey(nil)
	if err != nil {
		t.Fatal(err)
	}
	path := signedBundleFile(t, signer{pub: pub, priv: priv})

	d := NewInProcessDecider(InProcessConfig{BundlePath: path, SigningPubKey: pub, RequireVerified: true})
	if d.Integrity() != IntegrityVerified {
		t.Fatalf("integrity = %s, want verified", d.Integrity())
	}
	// The property is that the bundle was LOADED, not which rule fires: a
	// decision sourced from the local bundle means the strict posture admitted
	// it, where the refusal path leaves the decider at cold-start fail-open.
	got := d.Decide(context.Background(), toolCall("Bash", client.ToolShell, nil))
	if got.Source != sourceLocalBundle {
		t.Errorf("source = %q, want %q — a verified bundle must still be loaded under the strict posture",
			got.Source, sourceLocalBundle)
	}
	if got.FailOpen {
		t.Error("a loaded verified bundle must produce a real verdict, not a fail-open one")
	}
}
