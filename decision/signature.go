...
// VerifiedEpoch returns the epoch of a bundle whose signature verified, and
// ok=false otherwise.
//
// Epoch below reads the payload WITHOUT verifying it, which is only safe for a
// caller that has already checked the integrity — a rule enforced by a comment
// and one careful call site. This makes the safe reading the easy one.
func (b *Bundle) VerifiedEpoch(integrity Integrity) (int64, bool) {
...
//
// Hooks run as separate short-lived processes, so two can reach this at once.
// The read-then-write is not atomic across them: with a plain in-place write,
// a process pinning epoch 7 and one pinning epoch 5 can interleave so that 5
// lands last and lowers the floor. Writing to a temp file and renaming makes
// each write atomic, and re-reading the pin under the same operation keeps a
// racing lower value from winning — the worst remaining case is that a
// concurrent higher pin is written twice, never that a lower one survives.
func WriteEpochPin(bundlePath string, epoch int64) {
	...
	if _, err := tmp.Write([]byte(strconv.FormatInt(epoch, 10))); err != nil {
		tmp.Close()
		return
	}
	...
	// Re-check under the freshest read available: another process may have
	// advanced the floor past us while we were writing.
	if epoch <= ReadEpochPin(bundlePath) {
		return
	}
