...
// defaultFreshness is how long a loaded bundle is considered current. Past it,
...
// Server holds the loaded policy evaluator + Tier-1 secret detector and answers a
// DecisionRequest with NO I/O and NO network on the decision path (INV-3b). It is
// the in-memory engine behind InProcessDecider (ADR-0006): there is no socket, no
// listener, no resident daemon — the enforce hook constructs one, loads the local
// bundle, and calls decide directly. The mutex guards the atomic bundle swap so a
// decision sees either the whole old bundle or the whole new one.
type Server struct {
	mu        sync.RWMutex
...
func (s *Server) decide(req DecisionRequest) DecisionResponse {
	if req.Protocol != 0 && req.Protocol != ProtocolVersion {
		...
		// sourceFailOpenNoBundle lets the caller mark this FailOpen (E6-S7).
	...
	if eval == nil {
		// Cold start: no policy loaded yet → NO real verdict. VerdictUnknown records
		// honestly that OpenBox did not evaluate this call, and sourceFailOpenNoBundle
		// lets the caller mark the Decision FailOpen so the failure policy engages
		// (E6-S7 / E6-S3 INFO-1): fail-open (OD9 default) proceeds; fail-closed denies.
		// Never deny HERE — the engine does not know the policy, so the block decision
		// belongs to the failure policy, not this fail-open primitive.
	...
	// and bundle (OD-SYNC-10): it fires in the cold-start branch too, and never alters
	// resp.Evaluation — redact-and-continue is a proceed-path rewrite, orthogonal to
	// the deny/ask verdict. On a BLOCK verdict the tool never runs, so the enforce
	// hook simply ignores the attached redaction.
