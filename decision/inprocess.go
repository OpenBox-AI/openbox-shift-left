...
type Decision struct {
	...
	// window).
	// Stale reports that the policy on disk is older than the freshness window
	// — see DecisionResponse.Stale (OD-RF-4).
	Stale bool
...
// engine.decide path — the native builder/rules evaluator (ADR-0005) plus
...
// in microseconds. There is nothing to start.
//
// Safety properties:
//   - a missing/unreadable/invalid bundle → cold-start fail-open (engine
...
//   - staleness is surfaced (engine.decide flags Stale past Freshness);
...
type InProcessConfig struct {
	...
	// RequireVerified refuses to load a bundle whose signature did not verify
	// (OD-RF-3). Default false: the backend does not sign yet, so requiring it
	// today would leave every install bundle-less. An org that has deployed
	// signing sets it to turn detection into prevention.
...
// leaves the underlying engine at cold-start fail-open, so the enforce
...
func NewInProcessDecider(cfg InProcessConfig) *InProcessDecider {
	...
	srv := newEngine(engineConfig{Freshness: cfg.Freshness, Logger: log})
	...
	// OD-RF-3: an org that has deployed signing can require it. With this on,
	// a bundle that did not verify is NOT loaded, whatever the reason — so a
	// tampered or unsigned policy cannot be enforced as though it were the
	// org's. Combined with fail-closed that is real prevention; on its own it
	// degrades to no-bundle fail-open, which is still better than honouring a
	// policy nobody signed.
	//
	// Default off, because the backend does not sign yet: turning it on today
	// would leave every fleet bundle-less.
	if cfg.RequireVerified && integrity != IntegrityVerified {
		log.Printf("openbox enforce: local policy bundle at %s is %s and this org requires a verified bundle — "+
			"NOT loading it; decisions follow the failure policy (fail-closed denies, fail-open proceeds)", bp, integrity)
		return &InProcessDecider{srv: srv, integrity: integrity}
	}

	// Freshness is measured from the bundle file's mtime (OD-RF-4), so Stale
	// reports whether the policy this decision used is current — not how long
	// ago this short-lived process happened to load it.
	if fi, statErr := os.Stat(bp); statErr == nil {
		srv.SetBundleWrittenAt(fi.ModTime())
	}
	...
	case trusted != nil:
		...
		if integrity == IntegrityVerified {
			if epoch, ok := trusted.VerifiedEpoch(integrity); ok {
				WriteEpochPin(bp, epoch)
			}
		}
