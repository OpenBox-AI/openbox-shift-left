...
//
// Codex v0.145.0 exposes 11 events; this adapter wires the five below —
// the exact Claude Code parity set. PermissionRequest/Subagent*/`*Compact`/
// Stop are explicit non-goals.
...
// deliberate, gated exception: Prompt (the UserPromptSubmit text).
// `tool_input` is kept as raw bytes but is never decoded on the observe
// path (unlike Claude Code, Codex tool_input carries no structural
// file_path — apply_patch's input is the patch body itself, i.e.
// content), and `tool_response` (PostToolUse) has no field here at all,
// so neither can leak into an emitted event even by accident. Unknown/
...
type HookEvent struct {
	...
	// TranscriptPath is the filesystem path to this session's rollout
	// transcript (nullable on the wire; decodes to ""). A structural
	// locator, not content. Dereferenced only from the SessionEnd finops
	// path (usage.go), gated on ResolveFinops.
	...
	// ToolInput is retained only as an opaque blob for the enforce leg
	// (local, never-egressed decision input). The observe path never
	// decodes it.
	ToolInput json.RawMessage `json:"tool_input"`

	// SessionEnd. The embedded schema pins reason to the single value
	...
	// content-capture is opted in (Mapper.CaptureContent); with capture
	// off it is never copied onto an emitted event (parity with the CC
	// adapter).
...
//
// Enforce-only: this is the one place tool_input is decoded, and it runs
// only on the enforce PreToolUse path. The observe path never calls it
// (ToolInput stays an opaque json.RawMessage), so observe egress is
// unchanged. The decoded value is local-only: it feeds the in-process
// decider's match axis / redaction and is never egressed or logged
// (INV-2).
//
// Codex's PreToolUse tool_input is {"command":<string>} for both the
// shell tool (Bash — the command) and the file tool (apply_patch — the
// raw patch text), and updatedInput rewrites are re-parsed via
// updated_input["command"] (grounded @ rust-v0.145.0 core registry
// ApplyPatchHandler.pre_tool_use_payload + handlers/mod.rs
// updated_hook_command). A malformed/absent field yields "".
...
// fileText is the redactable body for the file class (apply_patch), which
// Codex carries in tool_input["command"] — the raw patch text (delta from
// Claude Code's content/new_string). Enforce-only and local-only (INV-2),
// like command().
