...
func TestTier2Decision_Codex(t *testing.T) {
	if d := tier2Decision(client.Evaluation{Verdict: client.VerdictUnknown}); !d.FailOpen {
		t.Error("an unknown T2 verdict must fold to FailOpen (no real verdict)")
	}
	if d := tier2Decision(client.Evaluation{Verdict: client.VerdictBlock}); d.FailOpen {
		t.Error("a real T2 BLOCK must be FailOpen=false")
	}
}
