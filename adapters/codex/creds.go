...
// ResolveEnforceTimeout resolves the hard per-call T1 decision budget:
// config first, env-if-parseable wins; <=0 yields 0 (⇒
// decision.DefaultDecisionTimeout); a positive value over
// maxEnforceTimeout is clamped. Clamping is adapter-owned (the devconfig
// resolver deliberately does not clamp) and derived for Codex — see the
...
func ResolveEnforceTimeout() time.Duration {
	...
	// can never overflow time.Duration (overflow-proof clamp, CC parity).
