...
func TestMap_LifecycleAndToolEvents(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "pretooluse bash → shell internal, tool_use_id pairing slot",
			hook:     HookPreToolUse,
			...
			wantSpan: &client.Span{SemanticType: "internal", Stage: "started", Function: "call-abc"},
		},
		{
			...
			wantSpan: &client.Span{SemanticType: "file_write", Stage: "completed", FileOp: "edit", Function: "call-def"},
		},
		{
		...
		},
		{
			...
			wantSpan: &client.Span{SemanticType: "internal", Stage: "started", Function: "call-jkl"},
		},
		{
			name:     "session end",
			hook:     HookSessionEnd,
...
func TestMap_IdentifiersBounded(t *testing.T) {
	...
	if got.Span == nil || len([]rune(got.Span.Function)) != maxIdentLen {
		t.Errorf("pairing id not bounded")
	}
...
// tool_use_id rides the pairing slot that feeds deriveID), while the same
