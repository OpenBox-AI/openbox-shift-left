...
func TestRedactToolInput_Codex(t *testing.T) {
	t.Run("swaps command, preserves other fields verbatim", func(t *testing.T) {
		...
		out := redactToolInput(orig, "clean-body")
		var m map[string]any
	...
	t.Run("no recognized content field ⇒ nil (nothing safe to rewrite)", func(t *testing.T) {
		if out := redactToolInput(json.RawMessage(`{"other":"x"}`), "y"); out != nil {
			t.Errorf("expected nil for no content field, got %s", out)
		}
	})
	t.Run("non-object ⇒ nil", func(t *testing.T) {
		if out := redactToolInput(json.RawMessage(`"a string"`), "y"); out != nil {
...
func TestApplyFailurePolicy_Codex(t *testing.T) {
	...
	if got := applyFailurePolicy(outage, FailOpen); got.Evaluation.WouldBlock() {
		t.Error("fail-open + outage must proceed")
	}
	if got := applyFailurePolicy(outage, FailClosed); !got.Evaluation.WouldBlock() {
		t.Error("fail-closed + outage must synthesize a HALT (deny)")
	}
	if got := applyFailurePolicy(realAllow, FailClosed); got.Evaluation.WouldBlock() {
...
func TestEnforceDecision_ObtainsRealVerdict(t *testing.T) {
	...
	if dec.FailOpen {
		t.Errorf("a real verdict must be FailOpen=false, got %+v", dec)
	}
}

// The adapter-owned clamps are DERIVED from the installed hot-hook timeout, not
// copied from Claude Code's constants (OD-SL7-T2-TIMEOUT).
func TestClampsDerivedFromInstalledTimeout(t *testing.T) {
	if installedHotHookTimeout != time.Duration(hotHookTimeoutSec)*time.Second {
		t.Errorf("installedHotHookTimeout must derive from the installer's hotHookTimeoutSec")
	}
	if maxEnforceHookBudget != installedHotHookTimeout-hookBudgetMargin {
		t.Errorf("maxEnforceHookBudget must be the installed timeout minus the margin")
	}
	if maxEnforceHookBudget >= installedHotHookTimeout {
