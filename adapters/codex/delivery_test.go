...
func TestDelivery_RetryIsBounded(t *testing.T) {
	...
	attempts := 0
	for i := 0; i < maxRecoveryAttempts+3; i++ {
	...
	if attempts > maxRecoveryAttempts+1 {
		t.Errorf("retried %d times, want at most %d", attempts, maxRecoveryAttempts+1)
	}
	if attempts < 2 {
...
func TestDelivery_RecoveryAttemptParsing(t *testing.T) {
	...
	for name, want := range cases {
		if got := recoveryAttempt(name); got != want {
			t.Errorf("recoveryAttempt(%q) = %d, want %d", name, got, want)
		}
	}
}
...
func TestEvidenceState_CountsOnlyCarriedOverEvents(t *testing.T) {
	...
	if got := (Spool{Dir: filepath.Join(dir, "nope")}).UndeliveredCount(); got != 0 {
