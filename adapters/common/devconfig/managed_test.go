...
func TestManaged_UnknownKeyMakesFileUnreadable(t *testing.T) {
	dir := t.TempDir()
	managedPath := filepath.Join(dir, "managed.json")
	writeJSON(t, managedPath, `{"enfoce":true,"locked":["enforce"]}`)
	t.Setenv(EnvManagedConfig, managedPath)
	t.Setenv(EnvConfigPath, filepath.Join(dir, "absent.json"))

	if st := Managed(); !st.Present || st.Readable {
		t.Errorf("a typo'd key must make the file unreadable, got %+v", st)
	}
}
