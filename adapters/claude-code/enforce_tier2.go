...
func runTier2(cctx context.Context, logger *log.Logger, m Mapper, ev *HookEvent) decision.Decision {
	...
	if err != nil {
		...
		// delivery failure (client.ErrDelivery). A synchronous escalation has
		// nothing to retry from — the spooled copy is the durable one — so
		// either way this degrades fail-open rather than guessing a verdict.
