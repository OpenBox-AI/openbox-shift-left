...
func RunHook(sub string, stdin io.Reader, stdout io.Writer, logger *log.Logger) {
	...
	if hook == HookSessionStart {
		// Best-effort session-start policy staleness check. Enforce-gated
		// so observe stays byte-identical (no network, no stdout on
		// SessionStart when enforce is off). Off the tool hot path; fully
		// fail-safe — it warns (fail-open) or marks the session stale
		// (fail-closed) but never blocks the session and never denies at
		// fetch time.
		if ResolveEnforce() {
