...
// maxRecoveryAttempts because the sweep re-read what the same pass just wrote.
...
func TestFlushDoesNotBurnRetriesInOnePass(t *testing.T) {
	...
	for want := 1; want <= maxRecoveryAttempts; want++ {
	...
	// The (maxRecoveryAttempts+1)th flush is the bounded give-up.
	if _, err := ad.Flush(context.Background(), "sess", offline); err != nil {
