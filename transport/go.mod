module github.com/openbox-ai/openbox-shift-left/transport

go 1.27.0

// The CONNECT/TLS front end is goproxy (D-OSS-1); everything that touches bytes
// or evidence is the EXISTING gateway relay, reused across the module boundary
// rather than forked. transport/guard_test.go enumerates exactly these two and
// fails on a third, so adding to the credential path stays a decision.
require (
